...
  options: RequestOptions;
  ...
  constructor(private store: Store<Stock>, private api: ApiClient) {
    ...
    deleteProduct(providerId: number, productId: number, selectedProducts: SelectedStock, total: number, numberOfChanges:number) {
    ...
    addProduct(stock: StockState, saleForm: FormGroup, providers: Provider[], selectedProducts: SelectedStock, total: number, numberOfChanges: number) {
        const product = stock.products.find(p => {
            const isTheSameProduct = p.name === saleForm.controls['product'].value;
            const isTheSameProvider = p.provider_id === parseInt(saleForm.controls['provider_id'].value);
        ...
        if (selectedProducts[selectedProviderId]) {
            selectedProducts[selectedProviderId].stock = [...selectedProducts[selectedProviderId].stock, addedProduct];
        } else {
            selectedProducts[selectedProviderId] = { stock : null, subTotal: 0, typeOfBuy: saleForm.controls['typeOfBuy'].value };
        ...
        selectedProducts[selectedProviderId].stock = selectedProducts[selectedProviderId].stock
          .map(p => {
        ...
        selectedProducts[selectedProviderId].stock = uniqBy(selectedProducts[selectedProviderId].stock,
    ...
    addSaleProduct(stock: StockState, saleForm: FormGroup, clients: Client[], selectedProducts: SelectedStock, total: number, numberOfChanges: number) {
        const product = stock.products.find(p => p.name === saleForm.controls['product'].value);
        const selectedClient = clients.find(c => c.id === Number(saleForm.controls['client_id'].value));
        ...
        if (selectedProducts[selectedClient.id]) {
            selectedProducts[selectedClient.id].stock = [...selectedProducts[selectedClient.id].stock, addedProduct];
            selectedProducts[selectedClient.id].saleDate = saleForm.controls['saleDate'].value;
        } else {
            selectedProducts[selectedClient.id] = { stock : null, subTotal: 0, paymentMethods: saleForm.controls['paymentMethods'].value, saleDate: saleForm.controls['saleDate'].value };
        ...
        selectedProduct.quantity = Number(selectedProduct.quantity) + Number(saleForm.controls['quantity'].value);
        selectedProducts[selectedClient.id].stock = selectedProducts[selectedClient.id].stock
          .map(p => {
            if (p.product === selectedProduct.product) {
                p.quantity = selectedProduct.quantity;
            }
        ...
        selectedProducts[selectedClient.id].stock = uniqBy(selectedProducts[selectedClient.id].stock,
                                                                 (p: AddedStock) => p.product.id);
